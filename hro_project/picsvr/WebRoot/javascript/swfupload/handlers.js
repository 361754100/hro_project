	...
	try {
		...
		if(serverData != null && serverData != undefined){
			...
			if(jsonObj.status === '1'){
				...
				progress.toggleCancel(false);
				progress.setStatus("上传失败");
				this.debug("错误代码: 上传失败, 文件名: " + file.name + ", 文件尺寸: " + file.size + ", 信息: " + jsonObj.message);
			}
			***/
		}else {
			progress.setError();
			progress.toggleCancel(false);
			progress.setStatus("上传失败");
			this.debug("错误代码: 上传失败, 文件名: " + file.name + ", 文件尺寸: " + file.size);
		}
	...
	try {
		...
		case SWFUpload.UPLOAD_ERROR.FILE_CANCELLED:
			...
			if (this.getStats().files_queued === 0) {
				document.getElementById('panel-body-defaultTip').style.display='block';
